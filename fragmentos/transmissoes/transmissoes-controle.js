...
async function navegarPara(secao) {
  ...
  if (breadcrumb)
    breadcrumb.innerText = `CORREDOR 777 // TRANSMISSÕES // ${label.toUpperCase()}`;
  ...
  if (status)
    setTimeout(() => {
      status.innerText = "STATUS: EM VIGÍLIA";
    }, 800);
}
...
async function montarDefinicoes() {
  ...
  try {
    ...
    // ─── EMBARALHAR ─────────────────────────────────────────────
    // Fisher-Yates — ordem fixa para esta carga de página,
...
async function abrirCollapse(url, elementoConteudo, tituloBotao) {
  ...
  document
    .querySelectorAll(".nota-conteudo.collapse-aberto")
    .forEach((aberto) => {
  ...
  if (!elementoConteudo.dataset.carregado) {
    try {
      ...
      reexecutarScripts(elementoConteudo);
    } catch {
...
function vincularEventos(raiz) {
  ...
  // Carregamento do KATEX
  if (typeof renderMathInElement === "function") {
    // Busca a classe .mat apenas dentro do fragmento injetado (raiz)
    raiz.querySelectorAll(".mat").forEach((elemento) => {
...
function renderizarMatematica() {
  // Verifica se o script do KaTeX já foi carregado para evitar erros
  if (typeof renderMathInElement === "function") {
    document.querySelectorAll(".mat").forEach(function (elemento) {
      renderMathInElement(elemento, {
        delimiters: [
          { left: "$$", right: "$$", display: true },
          { left: "$", right: "$", display: false },
