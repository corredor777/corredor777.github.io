...
// ============================================================
const dataIncidente = new Date(2026, 4, 13); // alterar seguindo o formato: (ano, mês-1, dia)
const hoje = new Date();
...
// ============================================================
const linhas = document.querySelectorAll(".painel tr, .colapso button"); // seleciona todas as linhas dentro dos painéis
...
linhas.forEach((linha) => {
  ...
  linha.addEventListener("mousemove", (e) => {
    ...
    if (Math.random() < velocidade * 0.1) {
      // gera um multiplicador aleatório para controlar a sensibilidade
  ...
  linha.addEventListener("mouseleave", () => {
    // detector para quando o mouse sai da área da linha
...
logBox.addEventListener("mouseleave", () => {
  // gatilho para quando o mouse sai da caixa de log
  const alertas = document.querySelectorAll(
    ".foco-vermelho, .foco-amarelo, .foco-azul",
  );
  alertas.forEach((alerta) => {
...
const particulas = Array.from({ length: 120 }, () => ({
  // Geração aleatória de propriedades para cada partícula
...
function desenharParticulas() {
  // função para desenhar e animar as partículas
...
const elementosStroboDuat = document.querySelectorAll(
  "#easter-tiamat, .duat-focus",
);
