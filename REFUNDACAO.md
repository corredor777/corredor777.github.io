...
Collections para diário e os três núcleos de notas, **em MDX** (decisão de 2026-07-06): casos especiais serão recorrentes — a intenção declarada é explorar cada vez mais WebGL e inserções de código por entrada — então toda entrada nasce podendo importar componente. Inserção visual = componente em `src/components/`, importado no MDX da entrada; `math`/`webgl` no frontmatter dizem ao layout o que carregar. `vesica-piscis` é o piloto desse padrão. Conversão das entradas HTML atuais: Claude Code escreve e roda o script de conversão em lote; **Bruno revisa entrada por entrada** — o conteúdo é a obra, conversão automática não se aprova sozinha. `CRT.astro` com `crt-effects` (religado via `astro:page-load`; adaptado pelo Claude Code — exceção dos efeitos). Slider randomizado das definições como island React pequena (aquecimento para o player da F3). KaTeX condicional. O SVG-inject da nav (`injetarSVGs`) vira SVG inline no build. Boot: roda só na entrada vinda de fora; navegação interna não re-boota (paridade com o vanilla).

Ordem interna da F2: **(a)** spike CRT × View Transitions — Claude Code adapta o `crt-effects`, monta `CRT.astro` mínimo com duas páginas dummy e mede se fósforo/rolo/jitter sobrevivem à navegação sem flash; **(b)** schema das collections + `content.config` (Bruno digita, Claude revisa); **(c)** conversão em lote + revisão entrada por entrada; **(d)** reconstrução das páginas (hub/boot, diário, notas com collapse + slider island + ensaios); **(e)** paridade (fades 150ms, status CARREGANDO→EM VIGÍLIA, nav bloqueada até o boot, mensagens de erro no vocabulário, modal Esc/clique-fora, fullscreen com voltar, Serpente bloqueada).

Decisão revista (2026-07-06): **MPA puro descartado** — recarregar a cada navegação muda o comportamento percebido (hoje a navegação interna é fluida) e mata a música contínua exigida pelo player de Transmissões (ver F3). Se as View Transitions brigarem com o CRT ou o cursor, o fallback é **página única**: o monitor continua trocando conteúdo client-side como hoje, com as collections alimentando os dados em build (controle enxuto no lugar das 638 linhas). O efeito ganha da conveniência, sempre (princípio 1).
...
- ~~**View Transitions vs. MPA puro.**~~ **Revisto (2026-07-06): MPA puro descartado** — quebra a fluidez atual das Transmissões e a música contínua. A escolha agora é **View Transitions vs. página única** (monitor client-side alimentado por collections); decide o spike CRT, primeiro movimento da F2.
